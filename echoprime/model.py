...
class EchoPrime:
    ...
    def process_mp4s(self,INPUT):
        ...
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"No mp4 videos loaded from {INPUT} (found {len(dicom_paths)} files). "
                "Please check path, file validity, or read errors."
            )
        stack_of_videos=torch.stack(stack_of_videos)
    ...
    def process_avis(self, INPUT):
        ...
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"No avi videos loaded from {INPUT} (found {len(video_paths)} files). "
                "Please check path or file validity."
            )
    ...
    def process_mp4s_limit(self, INPUT, max_videos):
        ...
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"No mp4 videos loaded from {INPUT} (tried {len(dicom_paths)} files)."
            )
        return torch.stack(stack_of_videos)
    ...
    def process_avis_limit(self, INPUT, max_videos):
        ...
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"No avi videos loaded from {INPUT} (tried {len(video_paths)} files)."
            )
        return torch.stack(stack_of_videos)
    ...
    def predict_metrics(self,study_embedding: torch.Tensor, k=50) -> dict:
