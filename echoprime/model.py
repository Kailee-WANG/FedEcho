...
class EchoPrime:
    ...
    def process_mp4s(self,INPUT):
        ...
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"在 {INPUT} 下未成功加载任何 mp4 视频（共找到 {len(dicom_paths)} 个文件）。"
                "请检查路径是否正确、文件是否为有效 mp4，或是否全部读取失败。"
            )
        stack_of_videos=torch.stack(stack_of_videos)

        return stack_of_videos

    def process_avis(self, INPUT):
        """
        与 process_mp4s 相同，但读取目录下的 AVI 文件（**/*.avi）。
        """
        video_paths = glob.glob(f'{INPUT}/**/*.avi', recursive=True)
        stack_of_videos = []
        for idx, path in enumerate(video_paths):
            try:
                pixels, _, metadata = torchvision.io.read_video(path)
                pixels = np.array(pixels)
                x = np.zeros((len(pixels), 224, 224, 3))
                for i in range(len(x)):
                    x[i] = utils.crop_and_scale(pixels[i])
                ...
                print("corrupt file", path, str(e))
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"在 {INPUT} 下未成功加载任何 avi 视频（共找到 {len(video_paths)} 个文件）。"
                "请检查路径或文件是否有效。"
            )
        return torch.stack(stack_of_videos)

    def process_mp4s_limit(self, INPUT, max_videos):
        """只读取并处理前 max_videos 个 mp4 文件，节省时间。内部调用与 process_mp4s 相同的处理逻辑。"""
        dicom_paths = glob.glob(f'{INPUT}/**/*.mp4', recursive=True)[:max_videos]
        stack_of_videos = []
        for idx, dicom_path in enumerate(dicom_paths):
            try:
                pixels, _, metadata = torchvision.io.read_video(dicom_path)
                ...
                print("corrupt file", str(e))
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"在 {INPUT} 下未成功加载任何 mp4 视频（共尝试 {len(dicom_paths)} 个文件）。"
            )
        return torch.stack(stack_of_videos)

    def process_avis_limit(self, INPUT, max_videos):
        """只读取并处理前 max_videos 个 avi 文件，节省时间。内部调用与 process_avis 相同的处理逻辑。"""
        video_paths = glob.glob(f'{INPUT}/**/*.avi', recursive=True)[:max_videos]
        stack_of_videos = []
        for idx, path in enumerate(video_paths):
            try:
                pixels, _, metadata = torchvision.io.read_video(path)
        ...
        if len(stack_of_videos) == 0:
            raise ValueError(
                f"在 {INPUT} 下未成功加载任何 avi 视频（共尝试 {len(video_paths)} 个文件）。"
            )
        return torch.stack(stack_of_videos)
    ...
    def generate_report_echonote(
        self,
        study_embedding: torch.Tensor,
        sections: Optional[List[str]] = None,
    ) -> str:
        """
        使用自定义的 echonote 风格块名列表生成报告，不依赖 utils.extract_section 的 [SEP] 结构。

        默认块名列表（可通过参数覆盖）：
          PATIENT/TEST INFORMATION, Indication, Height, Weight, BSA, BP, HR, Status,
          INTERPRETATION, Findings, LEFT ATRIUM, RIGHT ATRIUM/INTERATRIAL SEPTUM,
          LEFT VENTRICLE, RIGHT VENTRICLE, AORTA, AORTIC VALVE, MITRAL VALVE,
          TRICUSPID VALVE, PERICARDIUM, Conclusions.
        """
        import re
        ...
        # 简单的块提取：从 header 开始，到下一个 header 或文本末尾
        def _extract_block(report: str, header: str, all_headers: List[str]) -> str:
            # 优先按行首匹配 header，其次退回到简单的 .find
            pattern = r"(?m)^" + re.escape(header) + r".*?(?=^(" + "|".join(map(re.escape, all_headers)) + r")|\Z)"
        ...
        study_embedding = study_embedding.cpu()
        # 与原 generate_report 一致的视图加权逻辑，得到一个 no_view_study_embedding
        no_view_study_embedding = None
        for s_dx, sec in enumerate(self.non_empty_sections):
            cur_weights = [
                self.section_weights[s_dx][torch.where(ten == 1)[0]]
                for ten in study_embedding[:, 512:]
            ]
            tmp = study_embedding[:, :512] * torch.tensor(cur_weights, dtype=torch.float).unsqueeze(1)
            tmp = torch.mean(tmp, dim=0)
            tmp = torch.nn.functional.normalize(tmp, dim=0)
            # 这里只取一个全局 embedding，用第一个 section 的加权结果即可
            if no_view_study_embedding is None:
                no_view_study_embedding = tmp
        if no_view_study_embedding is None:
            # 回退：直接用 0:512 的均值
            no_view_study_embedding = torch.mean(study_embedding[:, :512], dim=0)
            no_view_study_embedding = torch.nn.functional.normalize(no_view_study_embedding, dim=0)

        similarities = no_view_study_embedding @ self.candidate_embeddings.T  # (N_candidates,)
        generated_report = ""

        for header in sections:
            # 为避免死循环，每个 header 独立复制一份 similarity，并逐个尝试候选
            sims = similarities.clone()
            used = torch.zeros_like(sims, dtype=torch.bool)
            extracted = ""
            while True:
                max_id = torch.argmax(sims)
                if used[max_id] or sims[max_id].item() == float("-inf"):
                    break
                used[max_id] = True
                predicted_section = self.candidate_reports[max_id]
                extracted = _extract_block(predicted_section, header, sections)
                sims[max_id] = float("-inf")
                if extracted:
                    break
            if extracted:
                generated_report += extracted + "\n\n"
